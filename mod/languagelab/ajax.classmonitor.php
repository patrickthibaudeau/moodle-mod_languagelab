...
else if ($eventType != '')
{
    ...
    else if($eventType == 'live_remove')
    {
        ...
        if($addLive = $DB->get_record_sql('SELECT * FROM {languagelab_user_event} WHERE languagelab = ? AND userid = ? AND type = ? AND data LIKE ?',
            array($languagelab->id, $studentid, 'live_add', '%'.$teacherlive.'%'))
           )
        {
    ...
    else if($eventType == 'liveclass_remove')
    {
        ...
        //If the live_add already exists, delete it and don't insert the live remove
        if($addLive = $DB->get_record_sql('SELECT * FROM {languagelab_user_event} WHERE languagelab = ? AND userid = ? AND type = ? AND data LIKE ?',
            array($languagelab->id, $studentid, 'liveclass_add', '%'.$teacherlive.'%'))
           )
        {
            $DB->delete_records('languagelab_user_event', array('id'=>$addLive->id));
            $insertEvent = false;
